...
jcfw_result_e jcfw_platform_init(void)
{
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, &s_als_i2c_handle));

    jcfw_platform_delay_ms(100); // NOTE(Caleb): See LTR303 docs
    err = jcfw_ltr303_init(&g_ltr303, s_als_i2c_handle, -1);
    JCFW_ASSERT_RET(err == JCFW_RESULT_OK, JCFW_RESULT_ERROR);
