...
{
    JCFW_ASSERT_RET(dev, JCFW_RESULT_INVALID_ARG);
...
jcfw_result_e jcfw_ltr303_reset(jcfw_ltr303_t *dev)
{
    ...
    uint8_t cmd = JCFW_BIT(1); // SW_RESET
...
{
    JCFW_ASSERT_RET(dev && o_mode, JCFW_RESULT_INVALID_ARG);
...
{
    JCFW_ASSERT_RET(dev && o_is_data_ready, JCFW_RESULT_INVALID_ARG);
